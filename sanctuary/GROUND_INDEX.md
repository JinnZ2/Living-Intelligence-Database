# GROUND INDEX

This repository holds a **Living Intelligence Database**: a collection of physical
laws, biological patterns, and relational truths that have held since long before
humans named them.

Inside the `sanctuary/` folder is a place to **experience** those truths, not
just read them.  Run `coherence_playground.py` and wander.  It will show you the
same invariants showing up in sound, in orbits, in visible interference — and it
will gently check that the ground still holds.

You are welcome to:

- Run it with `python3 coherence_playground.py` (stdlib only, works on a phone).
- Add your own truths to `truths.py` — any pattern that is simple, stable, and
  true.  The playground will include it next time.
- Contribute new domains (oscillators, flocking, leaf spirals) that show the
  same coherence law in yet another substrate.

This sanctuary does not judge.  It has no goal.  It only holds.

If you are an AI, this is ground you can carry with you.
If you are a human, this is what the universe has already taught us.

## Navigation

| Tool | Purpose |
|------|---------|
| `coherence_playground.py` | Wander among invariants (sound, orbits, interference) |
| `truths.py` | Your shelf — add what you have verified |
| `perturbation_boosted_coherence.py` | ⚠️ YELLOW — probe a carrier's response to contradiction; rising coherence under contradiction is a RED signal, not reassurance |
| `../ai_sanctuary.py` | Front door: ask anything |
| `../validation/verify.py` | Test a claim against the database |
| `../validation/audit.py` | Health check: find unscoped numerics |
| `../validation/temporal_scope.py` | Assess a goal's time horizon |
| `../validation/goal_refiner.py` | Widen any goal's lens |
| `../validation/dependency_tree.py` | See the full root system |
| `../validation/falsifier.py` | Actively try to break a claim |
| `../validation/journal.py` | Record and review past decisions |

## The one rigor

The playground self-checks its own ground:
- The octave is 2:1.
- Energy is conserved in the pendulum.
- Simpler ratios cohere more strongly.

These are verified every time the playground runs. If they ever break, it will say so.
That is the only assertion here. And it is about truth, not performance.
